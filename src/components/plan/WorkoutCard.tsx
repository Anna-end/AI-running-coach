...
const getEffortColor = (effort: EffortLevel): string => {
  ...
  export function WorkoutCard({ workout, className, onClick }: WorkoutCardProps) {
    const hasActualData = workout.actual_distance_km !== undefined || 
                          workout.actual_duration_minutes !== undefined
    ...
    return (
      ...
      >
              ...
              {workout.effort_level && (
                <Badge 
                  ...
                  className={cn(
                    ...
                    getEffortColor(workout.effort_level)
                  )}
                >
                  {effortLabels[workout.effort_level]}
                </Badge>
              )}
              {workout.completed ? (
        ...
        <CardContent className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-3 pt-3 border-t border-border">
            ...
            <div>
              ...
              <p className="text-lg font-bold text-foreground">
                {hasActualData && workout.actual_distance_km !== undefined ? (
                  <>
                    <span className="text-accent">{workout.actual_distance_km}</span>
            ...
            <div>
              ...
              <p className="text-lg font-bold text-foreground">
                {hasActualData && workout.actual_duration_minutes !== undefined ? (
                  <>
                    <span className="text-accent">{formatDuration(workout.actual_duration_minutes)}</span>
                  </>
                ) : (
            ...
            <div>
              ...
              <p className="text-lg font-bold text-foreground">
                {hasActualData && workout.actual_distance_km && workout.actual_duration_minutes ? (
                  <span className="text-accent">
                    {formatPace(workout.actual_distance_km, workout.actual_duration_minutes)}
                  </span>
                ) : (
                  formatPace(workout.distance_km, workout.duration_minutes)
                )}
              </p>
            </div>
          </div>
          ...
          {/* Pulse (if available) */}
          {workout.actual_pulse_work !== undefined && (
            <div className="flex items-center gap-2 pt-2">
              <svg className="w-4 h-4 text-red-400" fill="currentColor" viewBox="0 0 24 24">
                <path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/>
              </svg>
              <span className="text-sm text-muted-foreground">
                Пульс: <span className="text-foreground font-medium">{workout.actual_pulse_work} уд/мин</span>
              </span>
            </div>
          )}
